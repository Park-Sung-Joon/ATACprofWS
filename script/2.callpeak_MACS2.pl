...
print "## Readidng scale factors from " . $scale_file . "\n";
undef(%SCALES);
undef(@REP_IDs);
undef(%SAMPLE_IDs);
...
for($i=0; $i<scalar(@REP_IDs); $i++){
    ...
    $bed_file = $input_dir.  $this_ID . "_HG38.bed";
    if(!-e $bed_file){
...
foreach $key (keys %SAMPLE_IDs){
	...
	$command .= "awk -F \$'\\t' 'BEGIN {OFS = FS}{ if(\$6 == \"+\"){ t = \$2; \$2 = t - $move; \$3= t + $move }else if(\$6 == \"-\"){ t = \$3; \$2 = t - $move; \$3 = t + $move} print \$0 }' " . $this_BED . " | sort -V -k1,1 -k2,2 | gzip -c > " . $this_BED_sw;
	$command .= "\n\n";
    }
    ...
    $command .= "cat " . join(" ", @rep_bedFiles) . " | sort -V -k1,1 -k2,2 | gzip -c > " . $pooled_bed;
    $command .= "\n\n";
    # Peak call
